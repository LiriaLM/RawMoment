        ...
        await onSnapshot(collection(db, "posts"), (Querysnapshot) => {
            ...
            btnsLike.forEach((btn) => {
                btn.addEventListener('click', async (e) => {
                    ...
                    const doc = await getPost(id);
                    const post = doc.data();
                    if (post.personasLiked.includes(userEmail)) {//si mi correo dio like
                        //borrar de arreglo DISLIKE
                        //se elimina mi correo del arreglos usando filter ya que Pov solo agarra el ultimo
                        post.personasLiked = post.personasLiked.filter(email => email != userEmail);


                    } else { //si el correo no esta
                        //agregarlo a like LIKE
                        post.personasLiked.push(userEmail);
                    }

                    updatePost(id, post); // se actualiza en firestore (agregar o eiminar email)
...
function pintarPost(datos) {
    ...
    if (datos.length) {
        ...
        datos.forEach(doc => {
            ...
            const li = `
                <li class="list-group-item list-group-item-action mb-5"  style="background-color: #b488b8; color: black; border: 0px; border-radius: 7px ;">
                    <h6 style="color: #474747; " >${post.userEmail} publicó: </h6>
                    <p style="color: #474747; " >${formatoFechaHora(post.fecha.toDate())}</p>
                  <h3>${post.titulo}</h3>
                  <p>${post.descripcion}</p>
                  <img src="${post.imagenURL}" style="width:100%; padding: 5%;border-radius: 5px;" />
                  <button class="btn-delete" style="background-color: transparent; color: black; border: 0px;" data-id="${doc.id}"><i style="pointer-events: none;" class="bi bi-trash3"></i></button>
                  <button class="btn-edit" style="background-color: transparent; color: black; border: 0px;" data-id="${doc.id}"><i style="pointer-events: none;" class="bi bi-pencil-square"></i></button>
                  <button class="btn-like" style="background-color: transparent; color: black; border: 0px;" data-id="${doc.id}">${iconoLike}</button>
...
function pintarPostOtros(datos) {
    if (datos.length) {
        ...
        datos.forEach(doc => {
            ...
            const li = `
                <li class="list-group-item list-group-item-action list-group-item-dark mb-5" style="background-color: #b488b8; color: black; border: 0px; border-radius: 7px ; ">
                  <h6 style="color: #474747; " >${post.userEmail} publicó: </h6>
                  <p style="color: #474747; " >${formatoFechaHora(post.fecha.toDate())}</p>
                  <h3>${post.titulo}</h3>
                  <p>${post.descripcion}</p>
        ...
        datos.forEach(doc => {
            ...
            let li =""
                ...
                <li class="list-group-item list-group-item-action list-group-item-dark mb-5 " style="background-color: #b488b8; color: black; border: 0px; border-radius: 7px ; " >
                  ...
                  <p>${post.descripcion}</p>
                  <img src="${post.imagenURL}" style="width:100%; padding: 5%; border-radius: 5px;" />
                  <button class="btn-delete" style="background-color: transparent; color: black; border: 0px;" data-id="${doc.id}"><i class="bi bi-trash3"></i></button>
                  <button class="btn-edit" style="background-color: transparent; color: black; border: 0px;" data-id="${doc.id}"><i style="pointer-events: none;" class="bi bi-pencil-square"></i></button>
                  <button class="btn-like" style="background-color: transparent; color: black; border: 0px;" data-id="${doc.id}">${iconoLike}</button>
                   <p> A ${post.personasLiked.length} personas les gusta esto. </p>
                </li>
...
if (window.location.pathname === "/usuario.html" || window.location.pathname === "/likes.html" ) {
//Actualizar Post
